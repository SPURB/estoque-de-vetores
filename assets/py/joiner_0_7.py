...
secoes = ('../../blocos/imagens/','../../pictogramas/imagens/','../../infograficos/imagens/')
...
def json_maker(secao):
    encoding='UTF8'
    n_filename = "nome.txt"
    d_filename = "descricao.txt"

    have_full_img = False
    index_path = os.listdir(secao)
    json_list = []

    for imagem in index_path:
        name_path = secao + imagem + '/' + n_filename
        desc_path = secao + imagem + '/' + d_filename
        imagem = secao + imagem 
    ...
    base = open(secao +'base.js','w', encoding=encoding)
    base.write(json.dumps(json_list, indent=4,sort_keys=True,ensure_ascii=False))
    base.close()

for output in secoes: 
    json_maker(output)
